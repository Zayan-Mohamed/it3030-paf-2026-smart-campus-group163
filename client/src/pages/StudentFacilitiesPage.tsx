...
export const StudentFacilitiesPage = () => {
  const { token } = useAuth();
  ...
  return (
    <AdminFacilitiesLayout
      ...
      subtitle="Browse campus facilities, refine the list with filters, and start a booking request from the student portal."
    >
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-12 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Try adjusting the filters to find a matching facility.
              </p>
            </div>
          ) : (
                  ...
                  {facilities.map((facility) => (
                    <tr key={facility.id} className="bg-slate-50 text-sm text-slate-700">
                      ...
                      <td className="rounded-r-2xl px-4 py-4">
                        <Link
                          to={`/dashboard/student/bookings/new?facilityId=${facility.id}`}
